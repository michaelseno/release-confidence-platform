...
- New package `src/release_confidence_platform/evidence_retention/`: `models.py` (Pydantic models for `LegalHold`, `LegalHoldEvent`, `DisposalRecord`), `hold_repository.py` (`HoldRepository` — DynamoDB/S3 access for hold placement/release only, SK-guarded to `#LEGALHOLD#`), `disposal_repository.py` (`DisposalRepository` — DynamoDB write for disposal records only, SK-guarded to `#DISPOSAL#`, used exclusively by the Lambda), `service.py` (`RetentionService` — hold placement/release orchestration, depends on `HoldRepository` only), `identity.py` (ID generation, tag-key constants, SK-namespace constants), `commands.py` (new `rcp retention hold place|release|status` CLI group), `disposal_recorder.py` (Lambda handler logic invoked by the new Lambda, depends on `DisposalRepository` only). The `HoldRepository`/`DisposalRepository` split, and the mutually exclusive SK-write guards on each, are a direct fix for the write-path contradiction identified in architecture review — see Section 5.2.
...
evidence_retention/service.py (RetentionService) ──uses──> HoldRepository (SK-guarded to #LEGALHOLD# only)
   place_legal_hold / release_legal_hold  ──> LegalHold + LegalHoldEvent (MetadataTable, #LEGALHOLD#)
                                          ──> retags S3 object versions (rcp-legal-hold tag)
                                          ──> removes/restores ttl_disposal_at on matching DynamoDB items
...
- **`RetentionService` writes only to the `#LEGALHOLD#` sort-key namespace** (plus the two added fields — `custody_expires_at`, `ttl_disposal_at` — on existing records via `UpdateItem`) and only sets S3 object tags (never object content). It has no dependency on, and no code path capable of writing to, `#DISPOSAL#`: `RetentionService` is constructed with a `HoldRepository` instance only, and `HoldRepository` exposes no method that can construct a `#DISPOSAL#`-shaped write. `HoldRepository`'s own write methods additionally call `_assert_retention_sk()` before every `PutItem`/`UpdateItem`, which raises `AssertionError` if the target SK contains `#DISPOSAL#` — mirroring `_assert_phase7_sk` in `audit_platform_integrity/repository.py:49`, which guards Phase 7's writes against straying outside `#CERTJOB#`/`#CERT#`.
- **`#DISPOSAL#` is written exclusively by `evidenceDisposalRecorder`**, via a separate `DisposalRepository` class that `RetentionService` never imports. `DisposalRepository`'s write method calls the symmetric guard `_assert_disposal_sk()`, which raises `AssertionError` if the target SK contains `#LEGALHOLD#`. `evidenceDisposalRecorder` never deletes, updates, or reads any evidence content itself (S3 event payloads and DynamoDB stream `OLD_IMAGE` payloads carry everything needed); its only DynamoDB write is a conditional `PutItem` of a new `DisposalRecord`.
- This two-repository-class split with mutually exclusive SK-write guards is the code-level enforcement of Non-Negotiable Invariant 6 in the companion ADR. As with Phase 7's `_assert_phase7_sk`, this is a programming-error guard, not an IAM-enforced boundary — DynamoDB IAM cannot condition `PutItem` on sort-key substrings, only on the partition key. IAM is configured as defense-in-depth (Section 12) but the primary guarantee is structural, at the code level.
...
| `evidence_retention/hold_repository.py` (`HoldRepository`) | DynamoDB read/write for `LegalHold`/`LegalHoldEvent` only; S3 `ListObjectVersions`/`PutObjectTagging`/`GetObjectTagging` for hold re-tagging. Every write call is preceded by `_assert_retention_sk()`, an SK-write guard modeled directly on `_assert_phase7_sk` in `audit_platform_integrity/repository.py:49` — it asserts the target SK contains `#LEGALHOLD#` and does **not** contain `#DISPOSAL#`, raising `AssertionError` otherwise. This class has no `PutItem` method capable of constructing a `#DISPOSAL#`-shaped SK; the finding that motivated this split (Section 5.2) is closed structurally, not just by convention. |
| `evidence_retention/disposal_repository.py` (`DisposalRepository`) | DynamoDB write for `DisposalRecord` only — used exclusively by `disposal_recorder.py`. Every write call is preceded by the symmetric guard `_assert_disposal_sk()`, asserting the target SK contains `#DISPOSAL#` and does **not** contain `#LEGALHOLD#`. This class has no method capable of writing a `LegalHold`/`LegalHoldEvent`-shaped item. `RetentionService` never imports this class. |
| `evidence_retention/service.py` (`RetentionService`) | `place_legal_hold()`, `release_legal_hold()`, `get_hold_status()` — orchestrates `HoldRepository` calls only; has no dependency on `DisposalRepository` |
...
**Side Effects:** Writes `LegalHoldEvent` (immutable) + upserts `LegalHold` (current-state) to `ACTIVE`; re-tags every extant S3 object version under the audit identity's key prefixes (`raw-results/`, `intelligence/`, `reports/`, `integrity/`, each scoped by `{client_id}/{audit_id}/`) to `rcp-legal-hold=true`; `UpdateItem REMOVE ttl_disposal_at` on every DynamoDB item under `PK=CLIENT#{client_id}`, `SK begins_with AUDIT#{audit_id}`.
...
**Side Effects:** Writes `LegalHoldEvent` + updates `LegalHold` to `RELEASED`; re-tags S3 object versions back to `rcp-legal-hold=false`; restores `ttl_disposal_at = MAX(custody_expires_at, now)` on every matching DynamoDB item.
...
rcp retention hold place --client-id C --audit-id A --reason "..." --actor "..."
  ...
  ├─ RetentionService.place_legal_hold(...)   # uses HoldRepository only — no dependency on DisposalRepository
  ...
  │    ├─ Query MetadataTable: PK=CLIENT#C, SK begins_with AUDIT#A
  │    │    └─ for each item with ttl_disposal_at present: UpdateItem REMOVE ttl_disposal_at
...
src/release_confidence_platform/evidence_retention/
    ...
    hold_repository.py      # HoldRepository — DynamoDB + S3 access, SK-guarded to #LEGALHOLD# only (_assert_retention_sk)
    disposal_repository.py  # DisposalRepository — DynamoDB write, SK-guarded to #DISPOSAL# only (_assert_disposal_sk)
    service.py               # RetentionService — place/release orchestration (depends on HoldRepository only)
    disposal_recorder.py     # Lambda handler: DynamoDB Streams + S3 EventBridge event parsing (depends on DisposalRepository only)
...
- **`#DISPOSAL#` write-exclusivity — code-level guard, not an IAM boundary.** IAM's `dynamodb:PutItem` grant above is scoped to the table as a whole; DynamoDB IAM fine-grained access control conditions on the partition key (`dynamodb:LeadingKeys`) but cannot restrict `PutItem` to items whose sort key contains a given substring. The actual write-exclusivity guarantee — that only `evidenceDisposalRecorder` (via `DisposalRepository`) writes `#DISPOSAL#` items, and only `RetentionService` (via `HoldRepository`) writes `#LEGALHOLD#` items — is therefore enforced at the code level by the mutually exclusive `_assert_retention_sk()`/`_assert_disposal_sk()` guards described in Section 5.2/6, mirroring `_assert_phase7_sk` in `audit_platform_integrity/repository.py:49`. This is stated explicitly rather than implied by the IAM grant, which alone would not prevent a `HoldRepository` code change from accidentally writing a `#DISPOSAL#`-shaped item.
- **IAM scope for `RetentionService` (via `HoldRepository`):** requires `dynamodb:Query`/`UpdateItem`/`PutItem` on `MetadataTable` and `s3:ListBucketVersions`/`s3:GetObjectTagging`/`s3:PutObjectTagging` scoped to `RawResultsBucket`. Deliberately excludes `s3:DeleteObject`/`s3:PutObject` — the retention service tags, it never writes or deletes evidence content, preserving the Evidence Principle that raw evidence is the source of truth while within its authorized custody period.
...
- Unit-test the `_assert_retention_sk()`/`_assert_disposal_sk()` guards directly — including a negative test asserting each raises `AssertionError` when passed the *other* class's SK pattern — before any integration testing, mirroring the existing test coverage pattern for `_assert_phase7_sk` in the Phase 7 test suite.
