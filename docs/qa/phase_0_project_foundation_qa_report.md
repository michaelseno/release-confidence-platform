...
| AC-012 | Environment variables | Pass | `infra/serverless.yml` defines uppercase user-defined Lambda variables `STAGE`, `RAW_RESULTS_BUCKET`, `METADATA_TABLE`, and `LOG_LEVEL`; reserved Lambda-provided `AWS_REGION` is not configured by Serverless. |
