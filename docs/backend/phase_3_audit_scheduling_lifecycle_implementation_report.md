...
- `.venv/bin/python -m pytest` → `66 passed`.
- `.venv/bin/python scripts/validate_config.py --samples-dir configs/samples` → sample configs validated.
- From `infra/`: `npx serverless package --stage dev` → packaged successfully.
- From `infra/`: `npx serverless package --stage staging` → packaged successfully.
- From `infra/`: `npx serverless package --stage prod` → packaged successfully.
- From `infra/`: `npx serverless package --stage qa` → failed as expected with unsupported stage guard error.
